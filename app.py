...
# CSS para reducir el espacio en blanco superior y ocultar el menú por defecto de Streamlit
st.markdown("""
    <style>
           .block-container {
                padding-top: 1rem;
...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.text_input("Ingresa tu contraseña para entrar al Sistema Fiscal:", type="password", on_change=password_entered, key="password")
        return False
...
opcion_menu = st.radio(
    "Navegación:",
    ["📊 Panel General", "🧮 Calculadora de Impuestos", "📂 Extracción de XML (CFDI 4.0)", "📋 Catálogo y Claves SAT"],
    horizontal=True,
...
# ---------------------------------------------------------
    ...
    # Uso de columnas para reducir el ancho y centrar el módulo en pantalla
    col_izq, col_centro, col_der = st.columns([1, 2, 1])
    
    with col_centro:
        st.title("🧮 Calculadora de Retenciones")
        st.caption("Determina los importes directos e inversos para RESICO, Honorarios y Arrendamiento.")
        
        # --- GESTIÓN DE ESTADO PARA EL BOTÓN RESET ---
        ...
        regimen = st.selectbox(
            ...
            ["RESICO", "Arrendamiento", "Servicios Profesionales (Honorarios)", "Actividad Empresarial (Comercial)"]
        )
        
        # 2. Asignación de reglas: (Tasa ISR, Factor Ret IVA, IVA Sugerido)
        if regimen == "RESICO":
            opciones_ret = {
        ...
        escenario = st.radio("Configuración de Retenciones:", list(opciones_ret.keys()))
        ...
        with col_a:
            # Si el escenario especifica Tasa 0%, se deshabilita o fuerza el 0%
        ...
        with col_b:
            cantidad = st.number_input("Cantidad ($)*", min_value=0.0, step=100.0, key="calc_cantidad")
        ...
        if "Bruto" in tipo_calculo:
            ...
            factor_inverso = 1.0 + tasa_iva_dec - tasa_ret_iva_dec - tasa_isr_aplicable
        ...
        importe_neto = subtotal + importe_iva - importe_ret_iva - importe_ret_isr
        
        # --- RESULTADOS ---
        st.markdown("<br>", unsafe_allow_html=True)
        st.text_input("Subtotal", value=f"$ {subtotal:,.2f}", disabled=True)
        st.text_input("(+) Importe de IVA", value=f"$ {importe_iva:,.2f}", disabled=True)
        st.text_input("(-) Importe de Retención de IVA", value=f"$ {importe_ret_iva:,.2f}", disabled=True)
        
        etiqueta_isr = f"(-) Importe de Retención de ISR ({tasa_isr_aplicable * 100:.2f}%)" if tasa_isr_aplicable > 0 else "(-) Importe de Retención de ISR"
        st.text_input(etiqueta_isr, value=f"$ {importe_ret_isr:,.2f}", disabled=True)
        
        st.text_input("Importe Neto", value=f"$ {importe_neto:,.2f}", disabled=True)
        
        st.markdown("<br>", unsafe_allow_html=True)
        st.button("RESET", type="primary", on_click=limpiar_formulario, use_container_width=True)
...
# ---------------------------------------------------------
    ...
    archivos_xml = st.file_uploader("Seleccionar archivos XML", type=["xml"], accept_multiple_files=True)
    ...
    if archivos_xml:
        ...
        for archivo in archivos_xml:
            try:
                ...
                root = tree.getroot()
                datos_list.append({"Archivo": archivo.name, "Estado": "Leído correctamente"})
...
# ---------------------------------------------------------
    ...
    if os.path.exists(archivo_local):
        ...
        col_clave = next((col for col in df_catalogo.columns if 'clave' in col.lower()), df_catalogo.columns[0])
        col_desc = next((col for col in df_catalogo.columns if 'desc' in col.lower()), df_catalogo.columns[1] if len(df_catalogo.columns) > 1 else df_catalogo.columns[0])
        col_similares = next((col for col in df_catalogo.columns if 'similar' in col.lower() or 'palabra' in col.lower()), None)
        
        if busqueda:
            filtro = (
                df_catalogo[col_clave].astype(str).str.contains(busqueda, case=False) |
                df_catalogo[col_desc].astype(str).str.contains(busqueda, case=False)
            )
            if col_similares:
                filtro = filtro | df_catalogo[col_similares].astype(str).str.contains(busqueda, case=False)
        ...
        st.error("No se encontró el archivo 'catalogo.xlsx' en la carpeta del sistema.")
        st.info("Coloca tu archivo de Excel del SAT directamente dentro de la carpeta 'SistemaFiscal' y nómbralo 'catalogo.xlsx'.")